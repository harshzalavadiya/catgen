...
assembleApp.option('silent', false);
...
function registerAssembleTask(templateName, availableTemplates) {
	...
	function templateTaskHandler(currentTemplatePath) {
		...
		return requestDataFromUser(templateConfig)
			...
			.then(templateData => {
				let templateFolderGlob = path.join(currentTemplatePath, '**/*.tpl');
				let templateFolderGlobExe = path.join(currentTemplatePath, '**/*.keep');
				compileTemplateExe(templateData, templateFolderGlobExe, destination)
				return compileTemplateFolder(templateData, templateFolderGlob, destination);
			});
	}
}

function compileTemplateExe(templateData, templateFolderGlob, destination) {
	...
	return assembleApp.src(templateFolderGlob)
		...
		.on('end', () => {
			let successMessage = `${ICON_SUCCESS} Copied in ${destination}${NEWLINE}`;
